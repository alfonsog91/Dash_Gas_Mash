...
`model.js` is the contract authority for DGM's core output.

- `pGood` means the probability of receiving a good order within the next 10 minutes at this location under current modeled conditions.
- The heatmap visualizes midpoint `pGood` values only.
- Every probability output also carries `pGood_low`, `pGood_mid`, and `pGood_high`, generated by perturbing `λ` by `±30%`.
- Every probability output includes an `explain` payload with human-readable `merchantShare`, `residentialShare`, `relativeIntensity`, and `rainLiftPercent` fields.
...
In the shipped DGM contract, the exposed output pins $T = 10$ minutes so all visible probabilities stay directly comparable across the field.
...
The UI presents this as a range, not a single certainty. DGM computes `pGood_low` and `pGood_high` by perturbing modeled arrival intensity `λ` by `±30%` around the midpoint estimate.
...
- **Probability horizon**: Fixed at 10 minutes in the product contract so the probability field stays directly comparable across the map.
...
The UI shows a **10-minute probability range**, which corresponds to `pGood_low`–`pGood_high` around the midpoint `pGood_mid`. This is:
...
- Driven by public proxies: nearby merchants, time-of-night weighting, residential support, weather lift, and optional competition penalty.

When you click a point, the popup breaks it down:

- **merchantShare**: how much of the 10-minute probability is coming from nearby merchants
- **residentialShare**: how much is coming from surrounding homes and apartments
- **relativeIntensity**: how strong the point is versus the current view's reference intensity
- **rainLiftPercent**: how much the rain control is lifting modeled demand

Use the range to compare two parking choices under the same settings: a higher band means the model thinks that spot is better *relative to nearby alternatives*, while a wider band means more uncertainty.
